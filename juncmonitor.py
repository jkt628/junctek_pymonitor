...
logging.basicConfig(
    level=logging.INFO
)  # Configurar nivel de registro según tus necesidades
            ...
            async with bleak.BleakClient(device_mac) as client:
                if await client.is_connected():
                    ...
                    await client.start_notify(
                        read_characteristic_uuid, notification_callback
                    )
                    ...
                    logging.error(
                        f"Could not connect to device with MAC address {device_mac}"
                    )
